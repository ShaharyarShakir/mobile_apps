...
type PresetSelectorProps = {
  selectedPreset: ImagePresetKey;
  onSelectPreset: (preset: ImagePresetKey) => void;
};

const PRESET_KEYS: ImagePresetKey[] = ["high", "balanced", "maximum"];
...
export function PresetSelector({
  ...
  return (
    <View className="w-full">
      <Text className="mb-3 text-base font-bold text-black">Compression</Text>
      <View className="gap-3">
        {PRESET_KEYS.map((key) => {
          const detail = IMAGE_PRESET_DETAILS[key];
            ...
            >
              ...
              <View
                className={`mr-3.5 h-5 w-5 items-center justify-center rounded-full border ${
                  isSelected ? "border-black bg-black" : "border-neutral-300 bg-white"
                }`}
              >
                {isSelected && (
